...
MODULE NEMOH
    ...
    INTEGER(C_INT):: ct_Indiq_solver, j_case, nCase, j_fq, TD, iMaxThreads
    ...
    INTEGER(C_INT) :: is_interior_domain(Npanels) ! Array indicating whether or not a panel is in the body 
                                                  ! or the interior free surface domain.
    ...
    if (logp(LOG_INFO)) then
        write(logu,*) trim(logl(LOG_INFO)), " -> Initialisation X"
    endif
    ...
    IF(WhereToSave > 1) THEN
        ...
        ALLOCATE(InfluenceMatrixCache(NFA,NFA,iMaxThreads,2))
        ALLOCATE(CQCache(WhereToSave-1, 101), QQCache(WhereToSave-1, 101))
        ALLOCATE( SM1Cache(NFA, NFA,iMaxThreads), SM2Cache(NFA, NFA,iMaxThreads))
        ALLOCATE( SP1Cache(NFA, NFA,iMaxThreads), SP2Cache(NFA, NFA,iMaxThreads))
...
!        write(*,*) n_radiation, n_beta
    ...
    !$OMP PARALLEL DO PRIVATE(j_fq,j_case,j,TD,st,fi)

    DO j_fq=1,WhereToSave-1

        TD = OMP_GET_THREAD_NUM()+1
        call cpu_time(st)
...
!        write(*,*) CHECK_IS_WAVE(J, n_beta, n_radiation), GET_RAD_IDX(J, n_beta, n_radiation)
            ...
            CALL SOLVE_BVP(j,ID,TD,2.*PI/BodyConditions%Omega(j),&
