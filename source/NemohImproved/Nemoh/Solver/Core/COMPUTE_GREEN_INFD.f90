...
CONTAINS
    ...
    !-------------------------------------------------------------------------------!
    SUBROUTINE VAVNSINFD(deriv,KKK,ISYM,SolverVar,ZIJ,KdNum,TD)
        ...
        YGI=YG(I)
        ZGI=ZG(I)
!       REPALCE THE ORIGINAL VAVINFD SUBROUTINE
        PI4=ATAN(1.)
        PI=4.*PI4
        DPI=2.*PI
        QPI=4.*PI
...
!       REPALCE THE ORIGINAL VNSINFD SUBROUTINE
        PI4=ATAN(1.)
        PI=4.*PI4
        DPI=2.*PI
        QPI=4.*PI
        ...
        DO JJ=1,NJJ
            ...
            ELSE !2E
                ...
                DO IT=1,NQ
                    ...
                    ST=SIN(TETA)
                    DO L=1,4
                        ZIJNS(L)=AK0*(ZMIII+Z(KK(L))+ZI*((XGI-X(KK(L)))*CT+(YGI-Y(KK(L))*BX)*ST))
                        IF(REAL(ZIJNS(L)).GT.-25.)THEN
                            CEX(L)=CEXP(ZIJNS(L))
                        ELSE
                            CEX(L)=(0.,0.)
                        ENDIF
                        GZ(L)=GG(ZIJNS(L),CEX(L))
                    END DO
                    ZIJNS(5)=ZIJNS(1)
                    CEX(5)=CEX(1)
                    GZ(5)=GZ(1)
                    ...
                    ZVS=(0.,0.)
                    DO L=1,4
                        C1=ZIJNS(L+1)-ZIJNS(L)
                        IF(ABS(AIMAG(C1)).LT.EPS.AND.ABS(REAL(C1)).LT.EPS)THEN
                            ASD=(GZ(L+1)+GZ(L))*0.5
                            BSD=(CEX(L+1)+CEX(L))*0.5
                            CSD=ASD-(0.5/ZIJNS(L+1)+0.5/ZIJNS(L))
                        ELSE
                            CSD=(GZ(L+1)-GZ(L))/C1
                            ASD=(GZ(L+1)-GZ(L)+CLOG(ZIJNS(L+1)/ZIJNS(L)))/C1
        ...
        IF (KdNum.EQ.1) THEN
            IF(ISYM == 1) THEN
                PCOS=VSXP1*XN(I)+VSYP1*YN(I)+VSZP1*ZN(I)
                PSIN=VSXP2*XN(I)+VSYP2*YN(I)+VSZP2*ZN(I)
            ELSE
                PCOS=VSXM1*XN(I)+VSYM1*YN(I)+VSZM1*ZN(I)
                PSIN=VSXM2*XN(I)+VSYM2*YN(I)+VSZM2*ZN(I)
            ENDIF
