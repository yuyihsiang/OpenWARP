...
CONTAINS
    !
    SUBROUTINE SOLVE_BVP(ProblemNumber,ID,TD,Period,Switch_Kochin,&
        ...
        IMPLICIT NONE
        !       Inputs/outputs
        INTEGER :: ProblemNumber, Nintegration, NpanIsy, TD
        REAL,DIMENSION(:,:) :: NDS
        ...
        IF ((Depth.EQ.0.).OR.(kwave*Depth.GE.20.)) THEN
            ...
            ELSE
                ...
                IF (Indiq_solver == 0) CALL SOLVE_POTENTIAL_INFD_DIRECT(NVEL, SolverVar,TD)
                !           Solve using GMRES ?
                IF (Indiq_solver == 1) THEN
                    CALL SOLVE_POTENTIAL_INFD_ITERATIVE(NVEL, SolverVar,TD)
                END IF
        ...
        ELSE
            ...
            ELSE
                ...
                !           Solve with direct method ?
                IF (Indiq_solver == 0) CALL SOLVE_POTENTIAL_FD_DIRECT(NVEL,AMH,NEXP, SolverVar,TD)
                !           Solve using GMRES ?
                IF (Indiq_solver == 1) THEN
                    CALL SOLVE_POTENTIAL_FD_ITERATIVE(NVEL,AMH,NEXP, SolverVar,TD)
                    !STOP
                END IF
