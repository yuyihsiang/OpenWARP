...
def read_mesh(hdf5_data, custom_config):
    ...
    for i in range(mesh.n_panels):
        u = mesh.x[:, int(mesh.p[1, i])] - mesh.x[:, int(mesh.p[0, i])]
        v = mesh.x[:, int(mesh.p[3, i])] - mesh.x[:, int(mesh.p[1, i])]
        w1 = np.cross(u, v)
        a1 = 0.5*np.linalg.norm(w1)

        u = mesh.x[:, int(mesh.p[3, i])] - mesh.x[:, int(mesh.p[2, i])]
        v = mesh.x[:, int(mesh.p[1, i])] - mesh.x[:, int(mesh.p[2, i])]
        w2 = np.cross(u, v)
        ...
        mesh.xm[:, i] = (1./3)*(mesh.x[:, int(mesh.p[0, i])] + mesh.x[:, int(mesh.p[1, i])] + mesh.x[:, int(mesh.p[3, i])])*a1/mesh.a[i]

        mesh.xm[:, i]+= (1./3)*(mesh.x[:, int(mesh.p[1, i])] + mesh.x[:, int(mesh.p[2, i])] + mesh.x[:, int(mesh.p[3, i])])*a2/mesh.a[i]
